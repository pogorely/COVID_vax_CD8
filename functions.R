...
add_citeseq_tcrseq_metadata_revision<-function(path_to_aggr, postfix="aggr"){
  ctg<-fread(path_to_aggr)  
  bcds<-fread("libs_aggregate_rev/filtered_feature_bc_matrix/barcodes.tsv.gz",header=F)
  ftrs<-fread("libs_aggregate_rev/filtered_feature_bc_matrix/features.tsv.gz",header=F)
  mtr<-fread("libs_aggregate_rev/filtered_feature_bc_matrix/matrix.mtx.gz",skip=3,fill=F)
  TCRs<-fread("libs_aggregate_rev/filtered_contig_annotations.csv")
  ...
  setnames(ctg,"V1","barcode")
  #add dextramer barcodes and cell hashing statistics
  ftrs_ind<-ftrs[,((.N-38):.N),]
  mtrdex<-mtr[V1%in%ftrs_ind,,]
  mtrdex$dex<-ftrs[mtrdex$V1,V2,]
  mtr_casted<-dcast(mtrdex, V2~ dex,value.var = "V3")
  mtr_casted$barcode<-bcds[mtr_casted$V2]$V1
  mtr_casted<-na_to0(mtr_casted[,c("barcode",c(paste0("FB_dex",1:17),paste0("FB_dex",31:39),paste0("FB_hash",1:10),"FB_CD3","FB_CCR7","FB_CD45RA"))])
  dexb<-merge(ctg,mtr_casted,by="barcode",all=T)  
  #adds batch info. 
  dexb$batch<-paste0("batch_",sapply(strsplit(dexb$barcode,split = "-",fixed=T),"[[",2))
  #fix for batch1 donor hashing issues
  dexb[batch=="batch_1",FB_hash7:=FB_CD3,]#in batch 1 hashtag 7 is absent, but CD3 is there instead(as hashing antibody), so we substitute.  
  
  #get best_hashtag/batch columns
  hashes<-grep(pattern = "hash",names(dexb),value = T)
  dexs2<-grep(pattern = "dex",names(dexb),value = T)[-17] # 
  whichx<-function(x)
  {
    if (!is.na(sum(x))){
      if (sum(x)>0){
  ...
  dexb$hash<-(apply(prop.table(as.matrix(dexb[,..hashes]),margin = 1),MARGIN = 1,function(x)whichx(x>0.5)))
  dexb$bestdex16<-(apply(prop.table(as.matrix(dexb[,..dexs2]),margin = 1),MARGIN = 1,function(x)paste0(dexs2[whichx(x>0.3)],collapse="_")))
  #add donors/dex metadata.
  legend<-fread("metadata_rev/meta_revision.tsv")
  legend$batch<-paste0("batch_", legend$batch)
  setkey(x = legend,batch,hash,dex)
  
  dexb$spike<-legend[dexb[,.(batch,hash,dex=as.integer(gsub("FB_dex","",bestdex16))),],spike,]
  dexb$dextr<-legend[dexb[,.(batch,hash,dex=as.integer(gsub("FB_dex","",bestdex16))),],dex_id,]
  dexb$sample_category<-legend[dexb[,.(batch,hash,dex=as.integer(gsub("FB_dex","",bestdex16))),],sample_category,]
  dexb$donor<-legend[dexb[,.(batch,hash,dex=as.integer(gsub("FB_dex","",bestdex16))),],donor,]
  dexb<-dexb[!is.na(UMAP_1),]
  dexb$bestdex16_max<-(apply((as.matrix(dexb[,..dexs2])),MARGIN = 1,max))
  dexb<-dexb[!is.na(hash),,]
  dexb
